...
rates_plot <- ggplot(data.frame(N = 0:2025), aes(x=N)) +
  stat_function(fun = ipgr, color = "purple") +
  stat_function(fun = hr, color = "green") +
  geom_hline(yintercept = 0) +
  ylab("dN/dt") +
  theme_minimal()
...
P1_plot <- ggplot(data.frame(N = 0:2025), aes(x=N)) +
  stat_function(fun = ipgr_P1, color = "purple") +
  stat_function(fun = hr_P1, color = "green") +
  geom_hline(yintercept = 0) +
  ylab("dN/dt") +
  theme_minimal()
...
P6_plot <- ggplot(data.frame(N = 0:2025), aes(x=N)) +
  stat_function(fun = ipgr_P6, color = "purple") +
  stat_function(fun = hr_P6, color = "green") +
  geom_hline(yintercept = 0) +
  ylab("dN/dt") +
  theme_minimal()
...
Looking at the initial graph of P = 4, the domain of attraction for the zero equilibrium is any value of N between 0 and ~400, where the hunting rate first crosses the growth rate. For the largest equilibrium at N = 1500, the domain of attraction is between N = ~400 and N = 2000.  
...
**b. Draw a vertical dotted line where the zero equilibrium changes from locally stable to locally unstable.**  
